...
class DotNetFinder(importlib.abc.MetaPathFinder):
    ...
    def find_spec(klass, fullname, paths=None, target=None): 
        ...
        static readonly ConcurrentQueue<string> addPending = new ConcurrentQueue<string>();
        ...
        public static PyObject Import(string modname)
        {
            ...
            foreach (string name in names)
            {
                ...
                ModuleObject module = (ModuleObject)ManagedType.GetManagedObject(nested.Borrow());
                if (nested.IsNull() || module == null)
                {
