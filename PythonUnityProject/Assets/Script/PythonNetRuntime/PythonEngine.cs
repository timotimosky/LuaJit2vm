...
namespace Python.Runtime
{
    ...
    /// </summary>
    public partial class PythonEngine : IDisposable
    {
        ...
        public static Version MinSupportedVersion => new Version(3, 7);
        public static Version MaxSupportedVersion => new Version(3, 12, int.MaxValue, int.MaxValue);
        ...
        public static void Initialize(IEnumerable<string> args, bool setSysArgv = true, bool initSigs = false)
        {
            ...
            try
            {
                ...
                // BonusGamesHack: 这里调整为使用Unity的Resources读取py模块的文本
                string clr_py = Util.ReadResourcesText("clr.py");
                /*
                ...
                */
                ...
                // BonusGamesHack: 这里强制定义一个版本号，不使用Attribute
                var version = "1.0.0";
                /*
                // append version
        ...
        static void LoadSubmodule(BorrowedReference targetModuleDict, string fullName, string resourceName)
        {
            ...
            // BonusGamesHack: 这里调整为使用Unity的Resources读取py模块的文本
            string pyCode = Util.ReadResourcesText(resourceName);
            /*
            Assembly assembly = Assembly.GetExecutingAssembly();
        ...
        /// </summary>
        //既然现在EDITOR下python无法启动第二次，那不如直接屏蔽，这样可以加速关闭unity。
        ...
        static readonly List<ShutdownHandler> ShutdownHandlers = new List<ShutdownHandler>();
