...
namespace Python.Runtime
{
    ...
    {
        // BonusGamesHack: C#9.0的语法糖支持new的时候不写类型，这里需要调整下
        ...
        // BonusGamesHack: IL2CPP不支持动态生成类型
        ...
        private Type GetDispatcher(Type dtype)
        {
            ...
            // BonusGamesHack: IL2CPP不支持动态生成类型
            Debug.LogError($"[DelegateManager::GetDispatcher] 未知的类型 [{dtype.ToString()}]");
            return null;
    ...
    {
        // BonusGamesHack: 这里去掉源码中的只读属性，因为补丁需要修改此值
        ...
        private object? TrueDispatch(object?[] args)
        {
            ...
            {
                // BonusGamesHack: 这里对异常调用做调整，方便定位问题
                PythonException e = PythonException.FetchCurrentRaw();
