...
// OpenGL Core profile(桌面)或OpenGL ES(移动)RenderAPI实现。
//支持多种类型:Core, ES2, ES3
...
#elif UNITY_WIN
// On Windows, use gl3w to initialize and load OpenGL Core functions.
//  In principle any other library (like GLEW, GLFW etc.) can be used; here we use gl3w since it's simple and straightforward.
//在Windows上，使用gl3w初始化和加载OpenGL核心函数。
//原则上任何其他库(如GLEW, GLFW等)都可以使用;这里我们使用gl3w，因为它简单明了。
#	include "../gl3w/gl3w.h"
#elif UNITY_LINUX
...
void RenderAPI_OpenGLCoreES::DrawSimpleTriangles(const float worldMatrix[16], int triangleCount, const void* verticesFloat3Byte4)
{
	...
	//稍微调整投影矩阵，使其与D3D情况下的identity projection相匹配。
	...
	// Setup shader program to use, and the matrices 设置着色程序使用，和矩阵
...
#	endif // if SUPPORT_OPENGL_CORE
	...
	// Bind a vertex buffer, and update data in it 绑定顶点缓冲区，并更新其中的数据
