...
namespace Python.Runtime
{
    ...
    public partial class PyObject : DynamicObject, IDisposable, ISerializable
    {
        ...
        internal BorrowedReference obj => new BorrowedReference(rawPtr);

        public static PyObject None => new PyObject(Runtime.PyNone);
        internal BorrowedReference Reference => new BorrowedReference(rawPtr);
        ...
        ~PyObject()
        {
            if (!IsDisposed)
            {
                ...
                UnityEngine.Debug.LogError("PyObjectÖ´ÐÐÎö¹¹");
        ...
        public PyObject Invoke(PyTuple args)
        {
            ...
            if (NewReference.IsNull(r))
            {
                Console.Error.Write(
                    $"Failed to shutdown pythonnet: NewReference"
                );
              //  StolenReference s = new StolenReference(IntPtr.Zero);
                return new PyObject(IntPtr.Zero);
            }
            StolenReference  stolenRef= r.StealOrThrow();
            return new PyObject(stolenRef);
        }
