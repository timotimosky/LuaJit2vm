...
namespace Python.Runtime
{
    ...
    internal class Converter
    {
        ...
        internal static NewReference ToPython(object? value, Type type)
        {
            ...
            // BonusGamesHack: 这里不处理interface，而是使用默认的对象
            // 因为业务层可能会直接调用实现接口的类的函数，如果包装成interface会导致调用失败
            /*
            if (type.IsInterface)
            {
                ...
                case TypeCode.Int64:
                    {
                        ...
                        {
                            // BonusGamesHack: 转换Int64
                            long? long_value = Runtime.PyLong_AsLongLong(value);
                            if (long_value == null)
                            {
                                goto convert_error;
                            }
